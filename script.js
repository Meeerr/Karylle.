...
window.onload = () => {
  try {
    ...
    document.querySelectorAll(".cc-card").forEach(function (card) {
      ...
      var animatedDisplayDetails = cardBottom.animate(keyframeDisplayDetails, {
        duration: 100,
        iterations: 1,
        fill: "forwards"
      });
      animatedDisplayDetails.persist();
      card.dataset.cardPosition == 1
        ? animatedDisplayDetails.play()
        : animatedDisplayDetails.pause();
    });
  } catch (err) {
...
function animateCarousel(positionIncrements) {
  try {
    ...
    var hideDetailsAnimation = document
      ...
      .animate(keyframeDisplayDetails, {
        duration: 400,
        iterations: 1,
        easing: "ease-out",
        fill: "both"
    ...
    var keyframeRotateCarousel = [
      { transform: carouselStart },
      { transform: carouselEnd }
    ];
    var carouselContainer = document.querySelector(".cc-carousel-container");
    var containerAnimation = carouselContainer.animate(keyframeRotateCarousel, {
    ...
    document.querySelectorAll(".cc-card").forEach(function (card) {
      let cardAnimation = card.animate(keyframeRotateCard, {
        duration: animationDuration,
        iterations: 1,
        easing: "linear",
        fill: "both"
...
function getAnimatiopositionIncrement(cardPosition) {
  try {
    return cardPosition >= Math.round((cardCount + 1) / 2)
      ? cardCount + 1 - cardPosition
      : 1 - cardPosition;
  } catch (err) {
...
function getAnimationStartStop(positionIncrement) {
  try {
    ...
    startDegrees =
      positionIncrement < 0 ? animationPosition : Math.abs(animationPosition);
    endDegrees =
      positionIncrement < 0
        ? Math.abs(rotationArc * positionIncrement)
        : rotationArc * positionIncrement;
...
function setAnimationPositions(
  ...
  try {
    ...
    document.querySelectorAll(".cc-card").forEach(function (card) {
      let newPosition = getNewAnimationPosition(
        card,
        cardPosition,
        positionIncrements
      );
  ...
  try {
    ...
    newPosition =
      currentPosition == cardPosition
        ? 1
        : parseInt(currentPosition) + positionIncrements;
    ...
    if (newPosition > cardCount) {
      ...
      newPosition =
        positionIncrements < 0
          ? cardCount + 1 + positionIncrements
          : cardCount + newPosition + 1;
    }
    return newPosition;
  } catch (err) {
...
function setCardBackgrounds() {
  try {
    document.querySelectorAll(".cc-card").forEach(function (card) {
      ...
      cardTop.style.backgroundSize = getBackgroundImageSize(
        card.dataset.imageSize
      );
